...
def rnn_generator(split,batch_size,n_epochs=1,test=0,partition=0):
    ...
    for ecal,target in dl.batch_iter(partition,batch_size,n_epochs):
        # flattens array to shape (batch_size, 25, 400)
...
#TODO is this the right generator_data_pull?
...
def make_model():
    ...
    concat = merge([X,P_emb,M_emb],mode='concat',concat_axis=-1,name='pre_rnn')
